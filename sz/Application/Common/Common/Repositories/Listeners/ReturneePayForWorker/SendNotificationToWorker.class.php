...
namespace Common\Common\Repositories\Listeners\ReturneePayForWorker;
...
{
    /**
     * @param ReturneePayForWorkerEvent $event
     */
    public function handle(EventAbstract $event)
    {
        ...
        $content = "工单号：{$order['orno']}（{$service_type}}）\r\n"
			...
			您服务的{$order_product['cp_product_brand_name']}-{$order_product['cp_category_name']}工单已完成客户回访";
