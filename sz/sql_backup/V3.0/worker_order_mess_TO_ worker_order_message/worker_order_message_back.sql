...
(
   ...
   `name` varchar(255) NOT NULL COMMENT '昵称 ---del',
   `type` varchar(10) NOT NULL COMMENT '发起留言角色，A客服，B厂家，C维修商，D客户 ---del',
   `mess_role` varchar(50) NOT NULL DEFAULT '' COMMENT '留言双方角色，A：技工-客服，B：厂家-客服---del',
   primary key (id),
	 KEY `worker_order_id` (`worker_order_id`),
	 KEY `add_id` (`add_id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='工单留言记录 worker_order_mess';

INSERT into worker_order_message(
id, worker_order_id, content, create_time,`name`,`type`,`mess_role`) SELECT
id, worker_order_id, content, addtime,`name`,`type`,`mess_role` FROM worker_order_mess_backup;