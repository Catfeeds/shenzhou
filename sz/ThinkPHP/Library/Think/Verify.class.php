...
class Verify {
     ...
     * @param bool $is_base64 是否返回base64的数据 zjz
     * @return void
     */
    public function entry($id = '', $is_base64 = false) {
        // 图片宽(px)
        ...
        if ($is_base64) {
            ...
            $code_id = $this->codeSName();
            S($code_id, $verify_code, 1300);
    ...
    public function codeSName()
    {
        $s = md5(time().sprintf("%011d", rand(0, 99999999999)));
        if (S($s)) {
            $this->codeSName();
        }
        return $s;
