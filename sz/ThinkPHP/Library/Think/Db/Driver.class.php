...
abstract class Driver {
    ...
    protected function multiConnect($master=false) {
        // ******** 20171205 运行同一次脚本使用同一个connection，避免事务出现混乱 *******
        if (self::$connection) {
        ...
        if($this->config['rw_separate']){
            ...
            if($master)
                ...
                if(is_numeric($this->config['slave_no'])) {// 指定服务器读
                    ...
                    // ******** 20180103 随机获取连接 *******
                    $r = floor(mt_rand(1, $this->config['master_num']));   // 每次随机连接的数据库
                    // ******** 20180103  随机获取连接 *******
                }
            }
        }else{
        ...
        // ******** 20171205 运行同一次脚本使用同一个connection，避免事务出现混乱 *******
        self::$connection = $this->connect($db_config,$r,$r == $m ? false : $db_master);
        // ******** 20171205 运行同一次脚本使用同一个connection，避免事务出现混乱 *******
